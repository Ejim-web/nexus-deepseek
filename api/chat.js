...
async function verifyUser(req) {
  ...
  if (!header.startsWith("Bearer ")) {
    const error = new Error("Authentication token was not provided.");
...
function isAuthenticityRequest(text) {
  return /\b(
    ...
    fake\s+account|
    fake\s+invoice
  )\b/ix.test(String(text || ""));
}

function buildSystem(memories, authenticityMode) {
  const authenticityInstructions = authenticityMode
    ? `
...
If the image does not contain enough evidence, clearly say:
**"I cannot reliably determine authenticity from this image alone."**

For financial transactions, receipts, bank transfers, payment screenshots, gift cards, invoices, IDs, or other important documents, emphasize that visual analysis is not proof of a real transaction or legitimate document. The user should verify using the official service, transaction history, reference number, recipient account, or issuing organization.`
    : "";
...
The user may explicitly ask you to remember something. Use saved memories naturally.
...
async function getConversationHistory(db, uid, chatId) {
  if (!chatId) return [];
...
async function callOpenRouter({
  ...
  /*
   * Do not attach web search to image analysis.
   * The vision model should concentrate on the actual image.
   */
  if (research) {
    body.plugins = [
      {
...
function buildImageUserContent(message, imageData, imageMime, authenticityMode) {
  let imagePrompt = String(message || "").trim();
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let imageData = body.imageData || null;

    const imageMime = normalizeMime(
      body.imageMime
    );
     ...
     */
    if (imageData) {
      imageData = cleanBase64Image(imageData);
    }

    /*
     * Load memories and previous chat messages.
     */
     ...
     * IMAGE MODE
     *
     * Never send the web plugin together with the image.
     * First try the configured vision model.
     * If that fails, try Google's free vision model.
     */
    let data;
    let usedModel;
    ...
    if (imageData) {
      ...
      try {
        ...
        /*
         * Avoid making the same request twice if the configured
         * model already is the fallback.
         */
        if (
          VISION_MODEL ===
