...
const VISION_MODEL = process.env.NEXUS_VISION_MODEL || 'google/gemini-2.5-flash';
...
function shouldResearch(text) {
  return /\b(latest|today|current|recent|news|research|look\s*up|search|find\s+out|verify|sources?|price|prices|weather|market|2026|this\s+week|this\s+month)\b/i.test(text || '');
}

async function callOpenRouter({ messages, research, model = MODEL }) {
  const body = {
    model,
    messages,
    temperature: 0.2
