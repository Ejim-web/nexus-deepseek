...
function buildSystemPrompt(memories, hasImage, webSearchUsed) {
  // 1. DYNAMIC DATE INJECTION (Fixes the "2024" problem)
...
Today's date is ${today}. The current year is 2026. You must answer all questions with the awareness that it is currently 2026. Do not state that your knowledge cutoff is 2024. If you are asked about current events, rely on the LIVE WEB SEARCH results provided.
...
function shouldSearchWeb(message) {
  ...
  const explicit = [
    ...
    /\b2026\b/i, // Added for current year awareness
    /\bthis\s+year\b/i,
...
// 2. NEW GEMINI FALLBACK FUNCTION
    ...
    .map(m => {
      ...
      if (typeof m.content === "string") {
        ...
        text = m.content.map(part => {
          ...
          if (part.type === "image_url") return "[Image uploaded]"; // Gemini image handling is different
          return "";
        }).join("\n");
      }
      return {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          throw error; // Throw original OpenRouter error if both fail
        }
      }
